...
let url = 'http://localhost:3000/api/Spidermans';
let spider = {paht:'',title:''}
...
describe('test POST api', function(){
    it('post spiderma s to DB', function(done){
        request.post({url:url,form:spider}, function(a,b,c){
            //todo asserts here
            done();
        });
    });
...
describe('test DELETE api', function(){
    it('delete a spiderman', function(done){
        request.delete({url:url,form:spider}, function(a,b,c){
            //todo asserts here
            done();
        });
    });
